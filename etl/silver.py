...
class SilverLayer:
    ...
    def validar_cnpj(self, cnpj_series):
        ...
        logger.info(f"Validação CNPJ: {cnpj_validos}/{total} ({percentual:.1f}%) válidos")
    ...
    def validar_tipo_socio(self, tipo_socio_series):
        ...
        logger.info(f"Validação TIPO_SOCIO: {tipos_validos}/{total} ({percentual:.1f}%) válidos")
        return percentual > 50

    def aplicar_tipos_empresas(self, df):
        logger.info("Aplicando tipos de dados das empresas")
        try:
            ...
            logger.info("Tipos de dados das empresas aplicados com sucesso")
        except Exception as e:
            logger.error(f"Erro ao aplicar tipos para as empresas: {e}")
        return df

    def aplicar_tipos_socios(self, df):
        logger.info("Aplicando tipos aos socios")
        try:
            df['cnpj'] = df['cnpj'].astype(str)
            ...
            logger.error(f"Erro ao aplicar tipos aos socios: {e}")
        return df
                    ...
                    with z.open(filename) as f:
                        ...
                        try:
                            ...
                            if len(df_test.columns) >= 6:  # Se tem as colunas esperadas de empresas
                                f.seek(0)  # volta ao iní­cio do arquivo
                                ...
                                if self.validar_cnpj(df["cnpj"]):
                                    logger.info("Validação CNPJ passou para empresas")
                                else:
                                    logger.warning("Problemas de validação CNPJ para empresas")
                            ...
                            logger.warning(f"Arquivo {filename} não é CSV válido para empresas: {e}")
                            continue
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as z:
                for filename in z.namelist():
                    ...
                    with z.open(filename) as f:
                        # Tenta ler as primeiras linhas para detectar se é um CSV válido
                        try:
                            ...
                            if df_test.shape[1] >= 5:  # Se tem pelo menos 5 colunas
                                ...
                                if self.validar_cnpj(df["cnpj"]) and self.validar_tipo_socio(df["tipo_socio"]):
                                    logger.info("Validação passou para socios")
                                else:
                            ...
                            logger.warning(f"Arquivo {filename} não é CSV valido para socios: {e}")
                            continue
